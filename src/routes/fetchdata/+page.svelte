<script>
    ...
    try {
        ...
        if (response.ok) {
            ...
            showUpdateModal = false; // Close the modal on success
            selectedRow = null;
            await fetchData();  // Refresh data after update
        } else {
            const errorMessage = await response.text();  // Capture the error message from the backend
    ...
    onMount(async () => {
      // Fetch suppliers, buyers, and categories
        try {
